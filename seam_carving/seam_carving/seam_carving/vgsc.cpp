...
void VGSC::loadImage(IplImage *image)
{
	assert(image);
...
void VGSC::shrink(int num, bool speedup, bool isShow)
{
	assert(m_image);
	int w = m_image->width;
...
void VGSC::selectSeam(OffsetMap &accuMap, OffsetMap &dirMap, vector<CvPoint> &seamPos, bool isShow) const
{
	//从累计能量中得到最优化seam位置
	int w = accuMap.width();
...
void VGSC::calcAccumulateEnergyDirMap(bool isShow)
{
	//计算累计能量，只计算一次
	assert(m_gray);
	assert(m_dirMap);
	...
	for (int x = 1; x<w-1; ++x) {
		int D = gray_map[0][x-1];
		int E = gray_map[0][x];
		int F = gray_map[0][x+1];
		a_map[0][x] = abs(D - E) + abs(E - F) - abs(D - F);
		dir_map[0][x] = 0;
	}
	
	for (int y = 1; y<h; ++y) {
		//x = 0
		a_map[y][0] = a_map[y-1][0] + 2 * abs(gray_map[y][0] - gray_map[y][1]);
		dir_map[y][0] = 0;
		
		//x = w -1;
		a_map[y][w-1] = a_map[y-1][w-1] + 2 * abs(gray_map[y][w-1] - gray_map[y][w-2]);
		dir_map[y][w-1] = 0;	
...
void VGSC::reCalcAccumulateEnergy(OffsetMap &accuMap, OffsetMap &grayMap, 
...
void VGSC::reCalcAccumulateEnergySP(OffsetMap &accuMap, OffsetMap &grayMap, 
...
void VGSC::showSeamPos(void)const
{
...
void VGSC::calcSeamPos(const IplImage *idImage, const vector<CvPoint> &seamPos)
{
	//取得seam 在原来图像中的位置
